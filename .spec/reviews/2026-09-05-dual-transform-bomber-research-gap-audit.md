...
| LumioNativeCore | [TimerManager.advance](../../../LumioNativeCore/crates/lumio-timer/src/manager.rs) L526 按调用方提供的 Tick 推进；`modules/timer/README.md` 明确逻辑 Timer 不读墙钟，宿主断线窗口另管（该 README 已随 LumioNativeCore `9d7dd40` 删除，本行结论按当时代码作出，不回写） | 与调研“引信由逻辑 Tick 决定”的底层原则一致。没有证据表明炸弹必须另造墙钟定时器，也没有证据表明 Game 引信已经接入此模块 |
