...
- **能干什么**：
  ...
  **组件回池 Reset 契约（整块入池的清理闭环）**：实体销毁回池时，必须由生成的按类型克隆写回**全部**声明默认值，确保池化复用绝不残留上一任实体的脏数据。包含：
  - 容器字段（`SyncList` / `SyncDict` 等）：调用 `ResetForReuse()` 清空条目、重置内部状态与脏标记；
  - 框架组件字段与状态（如 `ObserverComponent`、`LogicTransform`、`ModelTransform` 等）：统一写回初始化默认值；
  - 事件与委托订阅：组件上的一切运行时事件订阅全部清空解绑；
  - 未登记普通字段治理：组件上未进入生成同步表的实例字段，**要么由结构断言禁止，要么显式声明 `[Local]` 属性进入重置表**统一重置；
  - 值类型字段的重置默认值必须来自组件类型声明处的显式初值，而不是语言底层的 `null` 或未定义未绑定态。
  - **验收基准**：「同类型销毁再创建，新实体任一字段等于声明默认值」。
