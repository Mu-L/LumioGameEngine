...
- **能干什么**：① 体素是密铺的规则立方体，**位置可以直接算出来**——用 **DDA 沿射线逐格步进**，不建空间划分树。② **命中的最小单位是 Block**：回答的是「哪一格、格里是什么 BlockId」，不返回比一格更细的东西。③ **三种检测，由体素侧统一提供，消费方不得自己再实现一套地形检测**：**射线检测**（起点 + 方向 + 最大距离 → 命中格坐标、BlockId、交点、面法线、距离）、**重叠检测**（一个盒子 → 相交的格列表；结果写进**调用方提供的缓冲**，体素侧不分配、不返回要释放的句柄，装不下就报 truncated 和实际总数）、**扫掠检测**（一个盒子 + 本帧位移 → 会不会撞、可行进比例 0~1、碰撞点、面法线）。**没撞上时可行进比例必须是 1.0**——一路畅通，整段位移都走得通；**不许写 0.0**，那是「一步也走不动」的意思，会把角色钉死在一片完全空旷的地方，而结构清零后的默认值恰好就是 0.0，所以 1.0 必须显式写出来。**答不了（Unresolved）时不给可行进比例**，调用方不许读那个字段。③a **v1 的形状只有一种：世界空间的轴对齐盒（AABB）**，由**盒心**与**三轴半长**两个点完整描述——**盒心就是位姿**，没有单独的位姿参数，也没有旋转和缩放。盒子**按值内联进请求结构**，不是一个指向未声明布局的指针（指针指到哪、那块内存什么布局、谁负责它的生命周期，契约里没写，四方就只能各自猜）。需要旋转包围盒时，调用方自己取一个能包住它的 AABB 传进来。任一半长为 0、负数或非有限值（NaN / 无穷）即**退化盒**，是非法请求：**体素侧必须在开始遍历之前逐分量检查、当场拒绝并返回 `degenerate_query_shape`**，**不许静默当成一个点（回 Hit），也不许静默当成 Miss**。两半都要堵：实测过同一个半长为 0 的盒子，盒心落在整数格边界上会被算成 Miss、落在格心会被算成 Hit——**只堵一半，另一半照样漏**。退化盒**不是第四种结局**，它不回 Hit / Miss / Unresolved 中的任何一个。「由调用方保证」是调用方的义务，不豁免被调方的检查义务：留成未定义行为，四方各自实现就会各挑一种结局，直接违背「两端必须同结果」。本条只管带形状的重叠与扫掠，射线不带形状不受它约束。形状集合到此为止，**加一种形状是契约变更**，要开 ADR 并同步 ABI。④ **每种检测都有三种结局，必须可区分**：**Hit**（命中）、**Miss**（确定没命中）、**Unresolved**（路径上有 Section 还没就位，答不了，返回是哪个 Section 键）。⑤ **查询带材质类过滤器**：角色移动只要 Solid，视线只要 Solid（水透过），判断「在不在水里」只要 Liquid——**阻挡与否只能查材质类表，物理实现里不许自带一份碰撞分支**。⑤a **过滤器就是一个 u32 位掩码**，一个材质类占一位，位序按材质类表的声明顺序从最低位起：**Solid = 第 0 位（值 1）、Liquid = 第 1 位（值 2）**，两类都要就按位或成 **3**。**掩码为 0 表示不匹配任何类，查询必定返回 Miss**——它不是「全选」，也不是错误码，想全选就显式写 3（Unresolved 的优先级仍高于 Miss）。**第 2 位及以上还没分给任何类，必须为 0**，置位即 `unknown_material_class`，不许当成「未来的类」静默忽略。新增材质类只能取下一个更高位，**已分配的位永不重排**——重排等于所有写死掩码的调用方一起改。掩码位是运行期过滤器，与 BlockType 的 ID 分段无关。⑤b **材质类表怎么进 Native：宿主在创建体素世界时注入**，来源就是官方方块目录 `materialClass` 那一列（LumioConfig 编译出的同一份配表），不是查询实现里的第二份表。**不给根表加目录槽**——目录是世界的构造状态不是逐次查询的输入，世界句柄已经是每个查询的第一个参数，多开一条入口就多一处能对不上的地方；**也不把目录做成查询参数**——每次带一遍会让同一帧的两次查询看到两份表，直接违背「两端必须同结果」。没注入目录的世界**不得回答任何查询**，三种查询一律以 `collision_behavior_not_from_material_table` 拒答；也不许回落到实现内置的默认表。**这跟 `unknown_material_class` 是两件事**：后者只用于「掩码触碰了保留位」与「有表但某一块解析不出材质类」，两者都能在有表的世界里单独发生，再拿它兼表示「整张表都没注入」，调用方就分不清到底是哪一件。解析不出材质类仍是 `unknown_material_class`，不许当成「不阻挡」。同一 Tick 内这张表不得变更。⑥ **两端必须同结果**：同一份世界、同一组输入，服务器和客户端算出同一个答案，预测与对账建立在这条上。⑦ **非体素物体 v1 不支持**：三种检测只回答体素，每条命中说的都是「哪一格、格里是什么 BlockId」，没有第二种命中目标，也没有任何把角色 / NPC / 掉落物注册进检测世界的入口。两条理由：ABI 的三个结果结构里**根本没有 target 判别字段**，「能区分撞的是方块还是物体」谁都做不到；而且按世界模型，角色 / NPC / 掉落物是**实体**，位姿与碰撞体归 ECS，把它们注册进体素世界等于在体素侧再开一份实体存储。**调用方自己合成**：地形那半问体素查询，实体那半问 ECS 侧的碰撞，取先发生的那个——这不算「自己再实现一套地形检测」，那条红线禁的是重造**地形**检测。要在引擎面统一回答「撞的是方块还是物体」得**先开 ADR**，v1 不留占位字段、不留半截入口。⑧ **物理查询只看「已发布的那一份」，永远不会替你去加载**：查询从发起那一刻的一份**不可变发布切面**取数，切面里没有的 Section 一律回 Unresolved；要它就位只有两条路——等改动层把它派发到 Ready，或者用区域 pin 声明它常驻。为什么非得是发布切面：客户端只可能拿到已发布的数据，服务端若允许查询看见还没发布的写入，两端同一条射线就会分叉。**它和逐格读/批量读不是同一个取数面**：读接口按坐标问世界的当前状态，物理只看发布切面——所以会出现**同一帧、同一个坐标，逐格读回 Ready 而射线 / 扫掠 / 重叠回 Unresolved**，这不是任一侧的 bug，是两个面的定义差异。物理的 Unresolved 只表示「不在这次的切面里」，**不区分**「从没加载」和「加载了但还没发布」，v1 也不给区分手段；调用方不许拿逐格读的 presence 去反推它、更不许据此推翻它继续通行。⑨ **语义在公共契约，函数签名在 ABI**：查询与返回的语义、缺块下的行为、过滤规则由 `voxel-world-v1.json` 定；具体 C 签名与结构布局属 `engine/abi/native-abi.json`。
...
- **做完的标准**：一条穿过 100 格的射线，在小世界和大世界里耗时相当——**耗时随经过的格数增长，与世界总方块数无关**；角色贴着墙走不会穿模也不会被卡住；射线穿过一片水打到水底石头，过滤器只要 Solid 时水不产生命中；把一个 Pending 的 Section 放进射线路径，得到 **Unresolved 并带上那个 Section 键**，既不穿过去也不挡住；给重叠检测一个只装得下 256 条的缓冲去罩 1000 个方块，返回 truncated 与实际总数 1000，而不是静默只给 256；服务器与客户端对同一条射线得到逐位相同的结果；掩码传 0 的扫掠**返回 Miss 而不是把两类都命中**，掩码置了第 2 位则返回 `unknown_material_class`；扫掠传一个半长为 0 的盒子被判为非法请求，而不是静默按一个点算。
...
| 把退化盒（半长为 0 / 负 / NaN）静默当成一个点或没命中 | **红线** | 永不——遍历之前当场拒绝并返回 `degenerate_query_shape`。两种静默结局同一个盒子都能到达（盒心落格边界回没命中、落格心回命中），只堵一半等于没堵 |
| 非体素物体（角色 / NPC / 掉落物）参与体素查询 | **不做（v1）** | 三个结果结构里没有 target 判别字段，「撞的是方块还是物体」谁都答不了；它们是实体，碰撞归 ECS。调用方自己把两半合起来。要在引擎面统一回答须先开 ADR |
...
| 可行进比例 | 扫掠检测的返回值：这一帧想走的距离里，实际能走多少（0 到 1）。撞墙前一刻停住就靠它。**没撞上就是 1.0**（全程畅通），不是 0.0；答不了时不给这个值 |
| 材质类过滤器 | 查询时说清「我只关心哪几类方块」。角色走路只看 Solid（掩码 1），判断在不在水里只看 Liquid（掩码 2），两类都要就是 3。一个类占一位；传 0 = 不关心任何类 = 必定 Miss，不是全选 |
