...
- **能干什么**：① 体素是密铺的规则立方体，**位置可以直接算出来**——用 **DDA 沿射线逐格步进**，不建空间划分树。② **命中的最小单位是 Block**：回答的是「哪一格、格里是什么 BlockId」，不返回比一格更细的东西。③ **三种检测，由体素侧统一提供，消费方不得自己再实现一套地形检测**：**射线检测**（起点 + 方向 + 最大距离 → 命中格坐标、BlockId、交点、面法线、距离）、**重叠检测**（一个盒子 → 相交的格列表；结果写进**调用方提供的缓冲**，体素侧不分配、不返回要释放的句柄，装不下就报 truncated 和实际总数）、**扫掠检测**（一个盒子 + 本帧位移 → 会不会撞、可行进比例 0~1、碰撞点、面法线）。③a **v1 的形状只有一种：世界空间的轴对齐盒（AABB）**，由**盒心**与**三轴半长**两个点完整描述——**盒心就是位姿**，没有单独的位姿参数，也没有旋转和缩放。盒子**按值内联进请求结构**，不是一个指向未声明布局的指针（指针指到哪、那块内存什么布局、谁负责它的生命周期，契约里没写，四方就只能各自猜）。需要旋转包围盒时，调用方自己取一个能包住它的 AABB 传进来。任一半长为 0、负数或非有限值即非法请求，**不许静默当成一个点，也不许静默当成 Miss**。形状集合到此为止，**加一种形状是契约变更**，要开 ADR 并同步 ABI。④ **每种检测都有三种结局，必须可区分**：**Hit**（命中）、**Miss**（确定没命中）、**Unresolved**（路径上有 Section 还没就位，答不了，返回是哪个 Section 键）。⑤ **查询带材质类过滤器**：角色移动只要 Solid，视线只要 Solid（水透过），判断「在不在水里」只要 Liquid——**阻挡与否只能查材质类表，物理实现里不许自带一份碰撞分支**。⑤a **过滤器就是一个 u32 位掩码**，一个材质类占一位，位序按材质类表的声明顺序从最低位起：**Solid = 第 0 位（值 1）、Liquid = 第 1 位（值 2）**，两类都要就按位或成 **3**。**掩码为 0 表示不匹配任何类，查询必定返回 Miss**——它不是「全选」，也不是错误码，想全选就显式写 3（Unresolved 的优先级仍高于 Miss）。**第 2 位及以上还没分给任何类，必须为 0**，置位即 `unknown_material_class`，不许当成「未来的类」静默忽略。新增材质类只能取下一个更高位，**已分配的位永不重排**——重排等于所有写死掩码的调用方一起改。掩码位是运行期过滤器，与 BlockType 的 ID 分段无关。⑤b **材质类表怎么进 Native：宿主在创建体素世界时注入**，来源就是官方方块目录 `materialClass` 那一列（LumioConfig 编译出的同一份配表），不是查询实现里的第二份表。**不给根表加目录槽**——目录是世界的构造状态不是逐次查询的输入，世界句柄已经是每个查询的第一个参数，多开一条入口就多一处能对不上的地方；**也不把目录做成查询参数**——每次带一遍会让同一帧的两次查询看到两份表，直接违背「两端必须同结果」。没注入目录的世界**不得回答任何查询**，也不许回落到实现内置的默认表；解析不出材质类就是 `unknown_material_class`，不许当成「不阻挡」。同一 Tick 内这张表不得变更。⑥ **两端必须同结果**：同一份世界、同一组输入，服务器和客户端算出同一个答案，预测与对账建立在这条上。⑦ 非体素物体（角色、NPC、掉落物）单独注册进检测世界，与地形一起参与查询，命中结果能区分撞的是方块还是注册物体。⑧ **语义在公共契约，函数签名在 ABI**：查询与返回的语义、缺块下的行为、过滤规则由 `voxel-world-v1.json` 定；具体 C 签名与结构布局属 `engine/abi/native-abi.json`。
